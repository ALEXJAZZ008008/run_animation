...
then
    ...
    cp -a "$INPUTPATH"'/cropped_input/dynamic_volumes/dynamic/.' "$OUTPUTPATH"'/momo_dynamic_cropped_input/'
fi
...
then
    ...
    cp -a "$INPUTPATH"'/reconstructed/dynamic_volumes/dynamic/.' "$OUTPUTPATH"'/momo_dynamic_reconstructed/'
fi
...
then
    ...
    cp -a "$INPUTPATH"'/cropped_input/saveDyn/.' "$OUTPUTPATH"'/momo_cropped_input/'
fi
...
then
    ...
    cp -a "$INPUTPATH"'/reconstructed/saveDyn/.' "$OUTPUTPATH"'/momo_reconstructed/'
fi
...
then
    ...
    cp -a "$INPUTPATH"'/ground_truth/cropped_xcat/.' "$OUTPUTPATH"'/ground_truth/'
fi
...
then
    ...
    cp -a "$INPUTPATH"'/evaluation/comparison_input/.' "$OUTPUTPATH"'/cropped_input/'
fi
...
then
    ...
    cp -a "$INPUTPATH"'/evaluation/comparison_reconstructed/.' "$OUTPUTPATH"'/reconstructed/'
fi
